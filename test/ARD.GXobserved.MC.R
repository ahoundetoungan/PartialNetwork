...
sourceCpp("ARD_to_linkprob.cpp")       # Should be "/path of the file/ARDmetro.cpp"
sourceCpp("DataForARD.cpp")              # Should be "/path of the file/DataForARD.cpp"
sourceCpp("Instruments.cpp")
##################################################################################
...
#To save results
lest1.1.1 <- lest1.1.2 <- matrix(NA,0,7)
lest1.2.1.1 <- lest1.2.2.1 <- matrix(NA,0,9)
lest1.2.1.2 <- lest1.2.2.2 <- matrix(NA,0,11)
lest2.1 <- matrix(NA, 0, 9)
lest2.2 <- matrix(NA, 0, 11)
lest3.1 <- matrix(NA, 0, 9)
lest3.2  <- matrix(NA, 0, 11)
...
colnames(lest2.1) <- colnames(lest3.1) <-  c("Intercept", paste0("X", 1:2), paste0("GX", 1:2), "alpha", "Weak", "Wu", "Sargan")
colnames(lest2.2) <- colnames(lest3.2) <- c("Intercept", paste0("X", 1:2), paste0("GX", 1:2), paste0("GXc", 1:2), "alpha", "Weak", "Wu", "Sargan")
...
for (l in 1:Iteration) {
  ...
  for (m in 1:M) {
    ...
    ##### Sample of individuals with ARD
    
    
    ###############################################################################
    ########################## Let's use updateGP function
    ...
    zeta0 <- 05
    z0 <- matrix(rvMF(N[m], rep(0, P)), N[m])
    ...
    #Initialization
    #Begin <- list(genz[sampleindex, ], genv, rowSums(G)[sampleindex], genb, geneta, genzeta)
    Begin<-list(z0,v0,d0,b0,eta0,zeta0)
    hparms = c(0, 1, 0, 1, 5, 0.5, 1, 1)
    Iter = 1000
    #Update the parameters
    Gparmsupdate <- updateGP(ARD, trait, Begin, vfixcolumn, bfixcolumn, Iter, fzeta = F)
    ...
    #The function computes the probability to be linked from the network model parameters
    distr <- dnetwork(Gparmsupdate, trait)
  ...
  lest1.1.1 <- rbind(lest1.1.1, c(sest1.1.1$coefficients[,1],sest1.1.1$diagnostics[,3]))
  ...
  lest1.1.2 <- rbind(lest1.1.2,  c(sest1.1.2$coefficients[,1],sest1.1.2$diagnostics[,3]))
  ...
  lest1.2.1.1 <- rbind(lest1.2.1.1, c(sest1.2.1.1$coefficients[,1],sest1.2.1.1$diagnostics[,3],cor((GY1all-GY1call),GXc0all)))
  ...
  lest1.2.1.2 <- rbind(lest1.2.1.2, c(sest1.2.1.2$coefficients[,1],sest1.2.1.2$diagnostics[,3],cor((GY1all-GY1call),cbind(GXc0all,G2Xcall))))
  ...
  lest1.2.2.1 <- rbind(lest1.2.2.1, c(sest1.2.2.1$coefficients[,1],sest1.2.2.1$diagnostics[,3],cor((GY1all-GY1call),GXcall)))
  print(paste("Iteration:",l,"Model without contextual effects, Gy not observed same draw and GX is instrument"))
  print(lest1.2.2.1[l,])
  ...
  lest1.2.2.2 <- rbind(lest1.2.2.2, c(sest1.2.2.2$coefficients[,1],sest1.2.2.2$diagnostics[,3],cor((GY1all-GY1call),cbind(GXcall,G2Xcall))))
  print(paste("Iteration:",l,"Model without contextual effects, Gy not observed, same draw and GX, GGX are instruments"))
  print(lest1.2.2.2[l,])
  ...
  lest2.1 <- rbind(lest2.1, c(sest2.1$coefficients[, 1], sest2.1$diagnostics[, 3]))
  ...
  lest2.2 <- rbind(lest2.2, c(sest2.2$coefficients[, 1], sest2.2$diagnostics[, 3]))
  print(paste("Iteration:", l, "Model with contextual effects"))
  print(lest2.2[l, ])
  ...
  lest3.1 <- rbind(lest3.1, c(sest3.1$coefficients[, 1], sest3.1$diagnostics[, 3]))
  ...
  lest3.2 <- rbind(lest3.2, c(sest3.2$coefficients[, 1], sest3.2$diagnostics[, 3]))
...
write.csv(sum.lest1.1.1, file = "ard.sum.lest1.1.1.csv")
write.csv(sum.lest1.1.2, file = "ard.sum.lest1.1.2.csv")
#write.csv(sum.lest1.2.1.1, file = "ard.sum.lest1.2.1.1.csv")
#write.csv(sum.lest1.2.1.2, file = "ard.sum.lest1.2.1.2.csv")
write.csv(sum.lest1.2.2.1, file = "ard.sum.lest1.2.2.1.csv")
write.csv(sum.lest1.2.2.2, file = "ard.sum.lest1.2.2.2.csv")
write.csv(sum.lest2.1, file = "ard.sum.lest2.1.csv")
write.csv(sum.lest2.2, file = "ard.sum.lest2.2.csv")
write.csv(sum.lest3.1, file = "ard.sum.lest3.1.csv")
write.csv(sum.lest3.2, file = "ard.sum.lest3.2.csv")
